...
import data from "../../smallCRM_TestData/data.json"
import { ExcelUtils } from "../../utilitys/excelUtils"

test("SystemTestScenarioTest",async ({page}) => {
    let userName1 = await ExcelUtils.readExcel("sheet1", 1, 1)

    let ran = Math.floor(Math.random()*1000);
    let url = data.url
    let name = data.name
    let email = data.email+`${ran}@gmail.com`
    let password = data.password+ran
    let rePassword = data.rePassword+ran 
    let contactNo = data.conNum
    let subject = data.sub
    let decriptionData = data.des
    ...
    /* Enter the name */
    await signupP.nameTF.fill(name+ran)
    /* Enter emailId */
    await signupP.emailTF.fill(email)
    /* Enter password */
    await signupP.passwordTF.fill(password)
     /* Enter re-password */
    await signupP.re_PasswordTF.fill(rePassword)
    /* Enter contect no */
    await signupP.contactNOTF.fill(contactNo)
    /* Select gender */
    ...
    await page.screenshot({path:`./screenshot/tickets${ran}.png`})
})